        ...
        where TContext : IDbContext
    {
            ...
            where TEntity : BaseEntity
        {
            ...
            if (!_services.ContainsKey(type))
            {
                _ = type switch
                {
                    { Name: "Material" } => _services[type] = new MaterialService(GetRepository<Material>()),
                    { Name: "User" } => _services[type] = new UserService(GetRepository<User>()),
                    { Name: "Course" } => _services[type] = new CourseService(GetRepository<Course>()),
                };
            }
            ...
            where TEntity : BaseEntity
        {
            ...
            if (!_repositories.ContainsKey(type))
            {
                _ = type switch
                {
                    { Name: "Material" } => _repositories[type] = new MaterialRepository(DbContext),
                    { Name: "User" } => _repositories[type] = new UserRepository(DbContext),
                    { Name: "Course" } => _repositories[type] = new CourseRepository(DbContext),
                };
            }
